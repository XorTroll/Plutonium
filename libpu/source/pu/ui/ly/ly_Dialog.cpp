...
namespace pu::ui::ly {
    ...
    void Dialog::OnStart() {
        ...
        this->opt_base_y = title_cnt_height;
    
        if(this->HasIcon()) {
            const auto icon_height = render::GetTextureHeight(this->icon_tex->Get()) + 2 * this->icon_margin;
            if(icon_height > (u32)this->opt_base_y) {
                this->opt_base_y = icon_height;
            }
            ...
            if(icon_title_width > this->dialog_width) {
                this->dialog_width = icon_title_width;
            }

            const auto icon_cnt_width = cnt_width + icon_width;
            if(icon_cnt_width > this->dialog_width) {
                this->dialog_width = icon_cnt_width;
            }

            const auto icon_opts_width = opts_width + icon_width;
            if(icon_opts_width > this->dialog_width) {
                this->dialog_width = icon_opts_width;
            }
        }
        
        if(this->dialog_width > render::ScreenWidth) {
            this->dialog_width = render::ScreenWidth;
        }

        this->dialog_height = this->opt_base_y + opt_row_count * this->opt_height + (opt_row_count - 1) * this->space_between_option_rows + this->opt_bottom_margin;
        if(this->dialog_height > render::ScreenHeight) {
            this->dialog_height = render::ScreenHeight;
        }

        this->dialog_x = (render::ScreenWidth - this->dialog_width) / 2;
        this->dialog_y = (render::ScreenHeight - this->dialog_height) / 2;
        this->opt_base_y += dialog_y;

        this->initial_fade_alpha = 0;
        this->initial_fade_alpha_incr.StartFromZero(this->fade_alpha_incr_steps, 0xFF);
    ...
    void Dialog::OnRender(render::Renderer::Ref &drawer) {
        auto use_opt_base_y = this->opt_base_y;
        ...
        for(u32 i = 0; i < opts_texs.size(); i++) {
            ...
            if((cur_opt_x + opt_width) >= pu::ui::render::ScreenWidth) {
                ...
                use_opt_base_y += this->opt_height + this->space_between_option_rows;
            }

            const auto opt_name_x = cur_opt_x + this->opt_h_margin;
            const auto opt_name_y = use_opt_base_y + ((this->opt_height - opt_name_height) / 2);
            ...
            if(this->selected_opt_idx == i) {
                ...
                drawer->RenderRoundedRectangleFill(over_clr, cur_opt_x, use_opt_base_y, opt_width, this->opt_height, this->opt_border_radius);
            }
        ...
        if(this->finish && !this->is_finishing) {
            this->finish = false;

            this->is_finishing = true;
            this->initial_fade_alpha_incr.StartToZero(this->fade_alpha_incr_steps, 0xFF);
        }

        if(this->is_finishing) {
            if(this->initial_fade_alpha_incr.Increment(this->initial_fade_alpha)) {
                this->SetActive(false);
            }
        }
        else {
            this->initial_fade_alpha_incr.Increment(this->initial_fade_alpha);
        }
    }

    void Dialog::OnInput(const u64 keys_down, const u64 keys_up, const u64 keys_held, const TouchPoint touch_pos) {
        ...
        if(!touch_pos.IsEmpty()) {
            ...
            for(u32 i = 0; i < this->opts.size(); i++) {
                ...
                if(touch_pos.HitsRegion(cur_opt_x, opt_base_y, opt_width, this->opt_height)) {
